...
autocomplete.addEventListener("click", (e) => {
  ...
  let words = inputSearch.value.split(" ");
  words[words.length-1] = e.target.innerHTML;
...
const extendAutocomplete = async (word) => {
  try {
    const url = `https://api.datamuse.com/sug?s=${word}&&v=es&&max=8`;
...
inputSearch.addEventListener("input", async (e) => {
  ...
  const values = await extendAutocomplete(words[words.length-1]);
  if (values.length === 0) {
    return 0;
  }
  ...
  values.forEach(v => {
    html += `<div class="auto-item" id="auto-item">${v['word']}</div>`
  })
...
const renderDocuments = (data) => {
  ...
  if (faceted) {
    let dataFacet = data.facet_counts.facet_fields.description
    ...
    for (let i = 0; i < dataFacet.length; i += 2) {
      if (dataFacet[i + 1] > 0) {
